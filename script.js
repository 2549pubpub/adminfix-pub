...
const SCRIPT_URL = "https://script.google.com/macros/s/AKfycby2HkqiRj8QNf443O6dkSdihCQABv5TaCA-Lax-qwY68elZfkGWyIqbyVJ_H4-JESDs/exec";
const PROMPTPAY_NUMBER = "0932365807"; // ⚠️ ใส่เบอร์โทรศัพท์พร้อมเพย์ หรือ เลขบัตรประชาชนของร้านคุณตรงนี้ได้เลยครับ!
...
function fetchDataFromGoogleSheets() {
    ...
    if(recentTable && allTable) {
        ...
        allTable.innerHTML = `<tr><td colspan="7" style="text-align:center; color:#aaa;">กำลังโหลด...</td></tr>`;
    }
...
function updateAppUI() {
    ...
    if(repairJobs.length === 0) {
        allTable.innerHTML = `<tr><td colspan="7" style="text-align:center; color:#aaa;">ไม่มีข้อมูลงานซ่อมในระบบ</td></tr>`;
        return;
    }
...
// 🌟 อัปเดตฟังก์ชันเปิด Modal ให้เจน QR Code ไปโชว์ที่ฝั่งขวาของหน้าต่างทันที
window.openEditModal = function(id) {
    ...
    // สั่งคำนวณและดึงรูป QR Code พร้อมเพย์ตามยอดราคาของงานนี้
    updateModalQR();
    
    document.getElementById('editModal').classList.add('show');
};

// 🌟 ฟังก์ชันคำนวณและเปลี่ยนรูป QR ตามการแก้ไขราคาแบบ Real-time บนหน้าจอ
window.updateModalQR = function() {
    ...
    // ประกอบลิงก์ API ยิงไปเอา QR Code สำเร็จรูปตามเบอร์พร้อมเพย์และราคาที่กรอก
...
// 🌟 อัปเดตระบบพิมพ์บิลให้เจน QR Code ฝังลงในกระดาษพิมพ์ใบเสร็จความร้อนอัตโนมัติ
window.printReceipt = function(id) {
    ...
    // ดึงรูป QR ใส่บิลกระดาษความร้อน
